...
def generate_report():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Missing graph ID. Please make sure you have built the graph"
            }), 400
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "generating",
                "message": "Report generation task started. Use /api/report/generate/status to query progress",
...
def get_generate_status():
    ...
    Request (JSON):
        {
            "task_id": "task_xxxx",         // Optional,  task_id returned from generate
...
def get_report_by_simulation(simulation_id: str):
    ...
    try:
        ...
        if not report:
            return jsonify({
                "success": False,
                "error": f"There is no report yet for this simulation: {simulation_id}",
                "has_report": False
            }), 404
...
def chat_with_report_agent():
    ...
    The Report Agent can use search tools to answer questions during conversations.
...
def get_report_sections(report_id: str):
    """
    Get the list of generated sections (output by section)

    The frontend can poll this interface to retrieve generated section content
...
def check_report_status(simulation_id: str):
    """
    Check if the simulation has a report, and its status
...
def get_agent_log(report_id: str):
    ...
    Provides real-time access to every step in the report generation process, including:
    - Report start, planning begins/finishes
    - Beginning of each section, tool calls, LLM responses, completion
...
def get_console_log(report_id: str):
    ...
    Provides real-time console output during report generation (INFO, WARNING, etc.).
    Unlike the agent-log interface which returns structured JSON logs,
