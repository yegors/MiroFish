"""
testProfileDoes the format generation comply withOASISRequire
verify:
1. Twitter ProfilegenerateCSVFormat
2. Reddit ProfilegenerateJSONdetailed format
"""
...
def test_profile_formats():
    """testProfileFormat"""
    print("=" * 60)
    print("OASIS Profileformat test")
    print("=" * 60)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if missing:
            print(f"\n   [mistake] Missing field: {missing}")
        else:
            print(f"\n   [pass] All required fields are present")
        
        # testReddit JSONFormat
        print("\n2. testReddit Profile (JSONdetailed format)")
        print("-" * 40)
        ...
        if missing:
            print(f"\n   [mistake] Missing required field: {missing}")
        else:
            print(f"\n   [pass] All required fields are present")
...
def show_expected_formats():
    """showOASISexpected format"""
    print("\n" + "=" * 60)
    print("OASIS expectedProfileFormat reference")
    print("=" * 60)
...
1,user1,User One,Tech enthusiast and coffee lover.,200,250,1000,2023-01-02"""
    ...
    print("\n2. Reddit Profile (JSONdetailed format)")
    print("-" * 40)
    reddit_example = [
        {
