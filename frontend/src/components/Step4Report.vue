            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section content generated, though the full section may still be in progress -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // Wait for the DOM update, then adjust scroll so the button stays in the same visual position.
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract summary statistics from the parsed text.
    const factMatch = text.match(/(?:Related Predicted Facts|\u76F8\u5173\u9884\u6D4B\u4E8B\u5B9E):\s*(\d+)/i)
    const entityMatch = text.match(/(?:Related Entities|\u6D89\u53CA\u5B9E\u4F53):\s*(\d+)/i)
    const relMatch = text.match(/(?:Relation Chains|\u5173\u7CFB\u94FE):\s*(\d+)/i)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    // Extract sub-questions without limiting the count.
    const subQSection = text.match(/### (?:Sub-Questions|\u5206\u6790\u7684\u5B50\u95EE\u9898)\n([\s\S]*?)(?=\n###|$)/i)
    if (subQSection) {
    ...
    // Extract core entities, including summaries and related fact counts.
    const entitySection = text.match(/### (?:\u3010\u6838\u5FC3\u5B9E\u4F53\u3011|Core Entities)\n([\s\S]*?)(?=\n###|$)/i)
    if (entitySection) {
      const entityText = entitySection[1]
      // according to "- **" Split solid blocks
    ...
    const relSection = text.match(/### (?:\u3010\u5173\u7CFB\u94FE\u3011|Relation Chains)\n([\s\S]*?)(?=\n###|$)/i)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    // Extract the query.
    const queryMatch = text.match(/(?:Search|\u67E5\u8BE2):\s*(.+?)(?:\n|$)/i)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Extract summary statistics.
    const nodesMatch = text.match(/(?:Total Nodes|\u603B\u8282\u70B9\u6570):\s*(\d+)/i)
    const edgesMatch = text.match(/(?:Total Edges|\u603B\u8FB9\u6570):\s*(\d+)/i)
    const activeMatch = text.match(/(?:Current Active Facts|\u5F53\u524D\u6709\u6548\u4E8B\u5B9E):\s*(\d+)/i)
    const histMatch = text.match(/(?:Historical\/Expired Facts|\u5386\u53F2\/\u8FC7\u671F\u4E8B\u5B9E):\s*(\d+)/i)
    ...
    // Extract current valid facts without limiting the count.
    const activeSection = text.match(/### (?:\u3010\u5F53\u524D\u6709\u6548\u4E8B\u5B9E\u3011|Current Active Facts)[\s\S]*?\n([\s\S]*?)(?=\n###|$)/i)
    if (activeSection) {
    ...
    // Extract related entities without limiting the count.
    const entitySection = text.match(/### (?:\u3010\u6D89\u53CA\u5B9E\u4F53\u3011|Related Entities)\n([\s\S]*?)(?=\n###|$)/i)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract the interview count (for example, "5 / 9 simulated agents").
    const countMatch = text.match(/\*\*(?:Interview Count|\u91C7\u8BBF\u4EBA\u6570):\*\*\s*(\d+)\s*\/\s*(\d+)/i)
    if (countMatch) {
    ...
    // Extract the selection reason for the interview targets.
    const reasonMatch = text.match(/### Reasons for selecting interviewees\n([\s\S]*?)(?=\n---\n|\n### Interview transcript)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Parse the selection reason for each individual.
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Format2: - Select name(index X):reason
        // For example: - Select parent_601(index 0):As a parent group representative...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*choose([^((]+)(?:[((]index\s*=?\s*\d+[))])?[::]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        // For example: - **parents_601(index 0)**:As a parent group representative...
        if (!headerMatch) {
        ...
        } else if (currentName && line.trim() && !line.match(/^Not Selected|^To sum up|^final choice/)) {
          // continuation of reason(Exclude closing summary paragraph)
          currentReason.push(line.trim())
        }
      }
      
      // Reasons to save the last person
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract title(like "student","education practitioner" wait)
      ...
      if (nameRoleMatch) {
        ...
        // Set the reason for the person's selection
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If the first question is preceded by"1.",Requires special handling
      ...
      // Extract answers - pointTwitterandReddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*Key Quotes|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // separationTwitterandRedditanswer
        const twitterMatch = answerText.match(/[TwitterPlatform answer]\n?([\s\S]*?)(?=[RedditPlatform answer]|$)/)
        const redditMatch = answerText.match(/[RedditPlatform answer]\n?([\s\S]*?)$/)
        ...
        // Platform fallback logic(Compatible with older formats:The case where there is only one platform tag)
        if (!twitterMatch && redditMatch) {
          // only Reddit answer,Copy as default display only when non-placeholder text
          if (interview.redditAnswer && interview.redditAnswer !== '(The platform did not receive a reply)') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(The platform did not receive a reply)') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // No sub-platform mark(Very old format),answer as a whole
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/Found\s*(\d+)\s*strip/)
    ...
    // Try to extract side information(if there is)
    ...
    // Try to extract node information(if there is)
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Latest key facts linked from temporal memory'),
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Sub-questions generated from drift queries'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'No current key memory yet'),
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No current active memory yet'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // for each question-Answer to Maintaining independent platform selection status
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(The platform did not receive a reply)' || t === '(The platform did not receive a reply)' || t === '[no reply]' || t === '(No reply from this platform)' || t === '[No reply]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "questionX:" or "questionX:" (Chinese format,New format for backend)
      // 2. "1. " or "\n1. " (number+point,Old format compatible)
      let matches = []
      let match

      // Try first "questionX:" Format
      const cnPattern = /(?:^|[\r\n]+)question(\d+)[::]\s*/g
      ...
      // If no match is found,Fallback to "number." Format
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // If no number is found or only one is found,return whole
      if (matches.length <= 1) {
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed:The first question returns the complete answer,The rest returns empty
    ...
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers(Non-placeholder text)And the content is different
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform switch button(Only shown if there are real dual-platform answers)
                  ...
                  // Expand/Collapse Button(Placeholder text is not displayed)
...
const activeStep = computed(() => {
  ...
  // if not active,Return the last done steps
...
const renderMarkdown = (content) => {
  ...
  // Remove the secondary title at the beginning(## xxx),Because the chapter title is already displayed in the outer layer
  ...
  // Process list - Supports sublists
  ...
  // Wrapping an unordered list
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Packaging ordered list
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // Clean all white space between list items
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Clean up the space after the list start tag
  ...
  // Handle newlines - Blank lines become paragraph breaks,Single newline becomes <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean up before and after block-level elements <br> Label
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // clean up <p><br> Following up on block-level elements(extra blank lines cause)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean up consecutive <br> Label
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean up the paragraph opening tag immediately following the block-level element. <br>
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering of non-consecutive ordered lists:When single item <ol> When separated by paragraph content,Keep the number increasing
...
const getLogLevelClass = (log) => {
  if (log.includes('ERROR') || log.includes('mistake')) return 'error'
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          // section_complete - Chapter generation completed
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              generatedSections.value[log.section_index] = log.details.content
              // Automatically expand newly generated chapters
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // if the task is completed,scroll to top;Otherwise scroll to the bottom to follow the latest posts
            if (isComplete.value) {
...
// Extract final answer content - from LLM response Extract chapter content from
const extractFinalContent = (response) => {
  if (!response) return null
  
  // Try to extract <final_answer> content within tags
  ...
  // try to find Final Answer: What follows(Support multiple formats)
  ...
  // try to find final answer: What follows
  const chineseFinalMatch = response.match(/final answer[::]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // If starting with ## or # or >, may be direct markdown content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // If content is long and contains markdown, return after removing the thought process
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove Thought: Thought process at the beginning
...
/* Console Logs - and Step3Simulation.vue Be consistent */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
