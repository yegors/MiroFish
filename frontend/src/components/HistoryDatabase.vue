<template>
  ...
  >
      ...
      >
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <!-- Show a hint when more files exist -->
            <div v-if="project.files.length > 3" class="files-more">
              +{{ project.files.length - 3 }} files
            </div>
          </div>
          <!-- Placeholder when no files are available -->
        ...
        <!-- Card title (uses the first 20 characters of the simulation requirement) -->
    ...
    <!-- Historical playback details modal -->
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="SelectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <!-- Playback limitation note -->
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 “Start Simulation” and Step 5 “Deep Interaction” must be launched live and do not support historical playback.</span>
            </div>
          </div>
        </div>
      </Transition>
...
// Card layout configuration - adjusted for a wider aspect ratio.
const CARDS_PER_ROW = 4
const CARD_WIDTH = 280  
const CARD_HEIGHT = 280 
const CARD_GAP = 24

// Dynamically compute container height styles.
const containerStyle = computed(() => {
  ...
  // Expanded state: compute height from the card count.
  ...
  // Compute required height: rows * card height + gaps + small bottom spacing.
  const expandedHeight = rows * CARD_HEIGHT + (rows - 1) * CARD_GAP + 10
  
  return { minHeight: `${expandedHeight}px` }
})

// Get card positioning styles.
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Compute the number of cards in the current row so each row stays centered.
    ...
    const x = offset * 35
    // Shift the starting position closer to the title while preserving spacing.
...
// Get a style class from round progress.
...
// format simulation_id show(before interception6Bit)
...
// Navigate to the analytics report page(Report)
const goToReport = () => {
...
// initialization IntersectionObserver
const initObserver = () => {
  if (observer) {
    observer.disconnect()
  }
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update the target status to be executed(Record the latest target state regardless of whether it is in animation or not)
        pendingState = shouldExpand
        
        // Clear previous anti-shake timer(The new scroll intent will overwrite the old one)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // If you are in animation,Only record status,Wait for the animation to finish
        if (isAnimating) return
        
        // If the target state is the same as the current state,No processing required
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Use anti-shake delay state switching,Prevent rapid flashing
        // Shorter delay when expanding(50ms),Longer delay when retracting(200ms)to increase stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check whether the pending execution status still needs to be executed(May have been overwritten by subsequent scrolls)
          ...
          // Unlock after animation completes,and check if there are any pending status changes
          setTimeout(() => {
            isAnimating = false
            
            // After the animation ends,Check if there is a new pending execution status
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay for a short period of time before executing,Avoid switching too quickly
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    {
      // Use multiple thresholds,Make detection smoother
      threshold: [0.4, 0.6, 0.8],
      // Adjustment rootMargin,The bottom of the viewport shrinks upward,Need to scroll more to trigger expansion
...
// Monitor routing changes,Reload data when returning to home page
...
onMounted(async () => {
  // make sure DOM Load data after rendering is complete
  await nextTick()
  await loadHistory()
  
  // wait DOM Initialize observer after rendering
  setTimeout(() => {
    initObserver()
  }, 100)
})
...
/* useCSSBackground pattern creates a fixed-spaced grid of squares */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Start positioning from the upper left corner,Only expands at the bottom when height changes,Does not affect existing grid positions */
...
.cards-container {
  ...
  /* min-height Depend on JS Dynamic calculation,Adaptable according to the number of cards */
}
...
.status-icon:nth-child(2).available { color: #F59E0B; } /* Environment setup - orange color */
...
.card-progress.in-progress { color: #F59E0B; }  /* in progress - orange color */
...
/* Low saturation color scheme - MorandiColor system */
