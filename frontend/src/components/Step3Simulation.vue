          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- DO_NOTHING: no action / stay idle -->
                ...
                <!-- Generic fallback for unknown types or content not handled above -->
...
// Computed
// Display actions in chronological order, with the newest entries at the bottom.
...
// Format simulated elapsed time from rounds and minutes per round.
...
const resetAllState = () => {
  ...
  stopPolling()  // Stop any previous polling timers.
}
...
const doStartSimulation = async () => {
  ...
  // Reset all state first so the previous run cannot affect this one.
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('Starting parallel dual-platform simulation...')
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Cleared previous simulation logs and restarted the run')
      }
...
// Track the previous round per platform to detect changes and print logs.
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect round changes per platform and print logs.
      ...
      // Extra check: if the backend has not updated `runner_status` yet, use platform completion flags.
...
const checkPlatformsCompleted = (data) => {
  // Return false when there is no platform data.
  ...
  // If at least one platform has completed, verify that every enabled platform has also completed.
  // Determine whether a platform was enabled from action counts or prior running state.
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // Return false when no platform was enabled.
  if (!twitterEnabled && !redditEnabled) return false
  
  // Check that every enabled platform has completed.
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Use `all_actions` to retrieve the complete action list.
      const serverActions = res.data.all_actions || []
      
      // Incrementally append new actions while deduplicating them.
      ...
      // Do not auto-scroll; let users inspect the timeline freely.
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Report generation request has already been sent. Please wait...')
    return
  }
