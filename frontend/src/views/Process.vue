      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">Graph build will start automatically after ontology generation finishes</p>
          </div>
          
          <!-- Building, but graph data is not available yet -->
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After upload, the LLM analyzes the documents and automatically generates an ontology structure suited for public-opinion simulation (entity types + relationship types).
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API Description</div>
                <div class="detail-content">
                  Using the generated ontology, the system chunks the documents and calls the Zep API to build a knowledge graph and extract entities and relationships.
                </div>
              </div>
...
// current projectID(possibly from'new'become realityID)
...
const currentPhase = ref(-1) // -1: Uploading, 0: Ontology is being generated, 1: Map construction, 2: Finish
...
const goToNextStep = () => {
  // TODO: Enter the environment setup steps
...
const initProject = async () => {
  ...
  if (paramProjectId === 'new') {
    // New project:from store Get the data to be uploaded
...
// Work on new projects - call ontology/generate API
const handleNewProject = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      // Update projectIDand data
      currentProjectId.value = response.data.project_id
      projectData.value = response.data
      
      // renewURL(Do not refresh the page)
      router.replace({
        name: 'Process',
      ...
      // Automatically start map construction
...
const startGraphPolling = () => {
  ...
  // Every 10 Automatically obtain spectrum data once every second
...
const fetchGraphData = async () => {
  try {
    // Get project information first to obtain graph_id
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        // Update progress is shown as complete
        buildProgress.value = {
          progress: 100,
          message: 'Build complete, loading graph...'
        }
        
        // Reload project data acquisition graph_id
        const projectResponse = await getProject(currentProjectId.value)
...
const renderGraph = () => {
  ...
  // Create nodeIDCollection used to filter valid edges
  ...
  // draw edges(Contains clickable transparent wide lines)
  ...
  // Transparent wide lines for clicks
  ...
  simulation.on('tick', () => {
    // Update the positions of all edges(Includes visible lines and transparent click areas)
