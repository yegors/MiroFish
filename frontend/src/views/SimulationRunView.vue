...
// Read `maxRounds` from the query during initialization so the child receives it immediately.
...
const handleGoBack = async () => {
  // Close the active simulation before returning to Step 2.
  ...
  try {
    // First try to close the environment gracefully.
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog('Failed to close the simulation environment. Trying forced stop...')
        try {
...
const loadSimulationData = async () => {
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      // Get simulation config so we can read `minutes_per_round`.
      try {
...
const loadGraph = async (graphId) => {
  // Avoid full-screen loading on automatic refresh while the simulation is running.
...
onMounted(() => {
  ...
  // Record the max-round configuration value read from the query.
  if (maxRounds.value) {
