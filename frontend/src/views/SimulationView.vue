...
/**
 * Check for a running simulation and close it.
...
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Try to shut down the simulation environment gracefully.
      try {
        ...
        if (closeRes.success) {
          ...
          // If graceful shutdown fails, try a forced stop.
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(`Simulation environment close error: ${closeErr.message}`)
        // If graceful shutdown throws, try a forced stop.
        await forceStopSimulation()
      }
    } else {
      // The environment is not running, but the process may still be active.
    ...
    // Failing to check environment state should not block the rest of the flow.
...
onMounted(async () => {
  ...
  // Check for a running simulation and close it when returning from Step 3.
